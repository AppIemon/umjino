...
<div class="dm-header">
  <span>💬 메시지</span>
  <button class="dm-new-btn" onclick="dmNewGroup()">+ 그룹</button>
...
<div class="dm-conv-item" onclick='dmOpenConv(${JSON.stringify(JSON.stringify(c))})'>
  ...
  <div class="dm-conv-info">
    ...
    const list = document.getElementById('dmList');
    if (list) list.innerHTML = '<div class="dm-empty">로드 실패</div>';
  }
}
...
async function dmLoadMessages(scrollBottom) {
  ...
  try {
    const res = await dmGet({ action: 'history', convId });
    const msgs = await res.json();
    const container = document.getElementById('dmMsgs');
    if (!container) return;
    if (!msgs.length) {
      container.innerHTML = '<div class="dm-empty">메시지 없음. 첫 메시지를 보내보세요!</div>';
      return;
    }
    container.innerHTML = msgs.map(m => dmMsgHTML(m)).join('');
...
function dmMsgHTML(m) {
  ...
  if (m.type === 'transfer') {
    ...
    return `<div class="dm-msg ${isMe?'dm-msg-me':'dm-msg-other'}">
      ${!isMe?`<div class="dm-sender">${escHtml(m.sender)}</div>`:''}
  ...
  const ctxAttr = `oncontextmenu="dmShowMsgCtx(event,${safeMid},${isMe},${safeContent})"
    ontouchstart="dmTouchStart(event,${safeMid},${isMe},${safeContent})"
    ontouchend="dmTouchEnd()"`;
  const editedMark = m.edited ? '<span style="font-size:.58rem;color:#555;margin-left:.3rem">(수정됨)</span>' : '';
  const deletedStyle = m.deleted ? 'opacity:.5;font-style:italic' : '';

  return `<div class="dm-msg ${isMe?'dm-msg-me':'dm-msg-other'}">
    ${!isMe?`<div class="dm-sender" onclick="showProfile('${escHtml(m.sender)}')" style="cursor:pointer">${escHtml(m.sender)}</div>`:''}
    <div class="dm-bubble" style="${deletedStyle}" ${ctxAttr}>${escHtml(m.content)}${editedMark}</div>
    <div class="dm-time">${time}</div>
  </div>`;
}
...
function dmOpenTransfer(convId, target) {
  const existing = document.getElementById('dmTransferOverlay');
...
<div style="background:#1a2a1a;border:2px solid rgba(46,204,113,.3);border-radius:14px;padding:1.2rem;max-width:360px;width:90%;color:white;max-height:70vh;overflow-y:auto">
  ...
  <div style="background:rgba(255,255,255,.07);border-radius:8px;padding:.5rem .7rem;margin-bottom:.4rem;cursor:pointer;transition:background .15s"
    ...
    <div style="font-size:.7rem;color:#555">${new Date(p.ts).toLocaleString('ko-KR')}</div>
...
function dmShowMsgCtx(e, msgId, isMe, content) {
  e.preventDefault();
  document.getElementById('dmMsgCtx')?.remove();
  ...
  menu.innerHTML = items.map(it =>
    `<div class="dm-ctx-item${it.danger?' danger':''}" onclick="${it.fn};document.getElementById('dmMsgCtx')?.remove()">${it.label}</div>`
  ).join('');
...
async function dmEditMsg(msgId, oldContent) {
  ...
  try {
    ...
    document.getElementById('dmMsgs')?.scrollTo(0, document.getElementById('dmMsgs').scrollHeight);
  } catch(e) { alert('서버 오류'); }
}
...
async function showProfile(nick) {
  let userInfo = null, rankInfo = null;
  try {
    const res = await fetchT('/api/profile?nick=' + encodeURIComponent(nick), null, 5000);
    userInfo = await res.json();
  } catch(e) {}
  try {
    const res = await fetchT('/api/ranking?nick=' + encodeURIComponent(nick), null, 5000);
    const d = await res.json();
    rankInfo = (d.top100||[]).find(r => r.nickname === nick) || null;
  } catch(e) {}
  ...
  const titleColor = userInfo?.titleColor || '#f1c40f';
...
<div style="background:linear-gradient(135deg,#1a3a28,#0d2018);border:2px solid rgba(241,196,15,.4);border-radius:16px;padding:1.5rem;max-width:320px;width:90%;color:white;text-align:center">
  ...
  <div style="font-size:1.3rem;font-weight:bold;color:#f1c40f;margin-bottom:.2rem">${escHtml(nick)}</div>
  ${title?`<div style="font-size:.82rem;color:${titleColor};margin-bottom:.3rem;font-weight:bold">[${escHtml(title)}]${isMe?` <button style="background:none;border:none;color:#888;cursor:pointer;font-size:.7rem" onclick="changeMyTitleColor()">🎨</button>`:''}</div>`:''}
  ${rankInfo
    ? `<div style="font-size:.85rem;color:#aaa">최고 칩: ${shortFmt(BigInt(rankInfo.maxChips||'0'))}</div>
       <div style="font-size:.78rem;color:#666;margin-top:.12rem">랭킹 ${rankInfo.rank}위</div>`
    : '<div style="font-size:.78rem;color:#555">랭킹 없음</div>'}
  ${!isMe ? `
  <div style="margin-top:.7rem;border-top:1px solid rgba(255,255,255,.1);padding-top:.6rem">
    ...
    <div style="display:flex;gap:.4rem;justify-content:center">
      ...
      <button class="btn-primary" style="font-size:.85rem" onclick="doInvest('${escHtml(nick)}')">투자</button>
    </div>
  </div>` : ''}
...
async function doInvest(target) {
  ...
  if (amt > chips) { alert('칩 부족'); return; }
  try {
    const res = await fetchT('/api/invest', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nickname: sessionNickname, token: sessionToken, target, amount: amt.toString() })
    }, 8000);
    const d = await res.json();
    if (!res.ok) { alert(d.error || '오류'); return; }
    chips = BigInt(d.newChips); chipDist = computeGreedyDist(chips);
...
async function dmLoadInboxInline() {
  ...
  try {
    ...
    if (!convs.length) { list.innerHTML = '<div class="dm-empty">대화가 없습니다</div>'; return; }
    list.innerHTML = convs.map(c => `
<div class="dm-conv-item" onclick='showDM();dmOpenConv(${JSON.stringify(JSON.stringify(c))})'>
  <div class="dm-conv-icon">${c.type==='group'?'👥':'💬'}</div>
  <div class="dm-conv-info">
    <div class="dm-conv-name">${escHtml(c.name)}${c.unread>0?`<span class="dm-unread-dot">${c.unread}</span>`:''}</div>
    <div class="dm-conv-last">${c.lastMsg?escHtml(c.lastMsg.content.slice(0,40)):'메시지 없음'}</div>
  ...
  } catch(e) { const l = document.getElementById('dmListInline'); if(l) l.innerHTML = '<div class="dm-empty">로드 실패</div>'; }
}
